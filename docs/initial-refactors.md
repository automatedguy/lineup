...
**19. Page Element Map — structured JSON output from WebDescriber (LINEUP-18)** — WebDescriber now outputs a structured JSON **Page Element Map** instead of free-text description. The map groups elements by page section (header, main content, footer), with each element having a `description`, `type`, and `method`. Action methods align with Stagehand's suggested actions (`click`, `fill`, `type`, `press`, `scroll`, `select from dropdown`) plus a custom `assert-visible` for non-actionable elements (text, headings). The `PageDescription` type changed from `description: string` to `elementMap: PageElementMap`. Ollama's `format` parameter constrains the vision model to output valid JSON with an enum on the method field. Prompt rules enforce section grouping, top-to-bottom/left-to-right ordering, and no-skip of elements. Known vision model limitations: can misidentify icons (e.g., camera as voice search), place elements in wrong sections, or skip elements entirely. Future enrichment with `observe()` (DOM ground truth) can address these.
